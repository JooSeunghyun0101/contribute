    ...
    <div className="card-lg">
      ...
      <div style={{marginTop:18,padding:14,background:'var(--ok-orange-50)',borderRadius:10,display:'flex',gap:10,alignItems:'flex-start'}}>
        ...
        <div style={{fontSize:12,lineHeight:1.6,color:'var(--ok-brown)',flex:1}}>
          <b>AI 검토</b> · 이 과업은 평가자(김철수 부장)의 승인 후 매트릭스에 반영됩니다. 현재 총 가중치는 <b>{MM.myTasks.reduce((s,x)=>s+x.weight,0) + (form.weight||0)}%</b>이며, 100% 초과 시 기존 과업 가중치를 조정해야 합니다.
        </div>
      </div>
      ...
      <div style={{display:'flex',flexDirection:'column',gap:14}}>
        <div className="card">
          ...
          <div style={{marginTop:10,display:'flex',alignItems:'center',gap:10,padding:10,background:'var(--bg-muted)',borderRadius:8}}>
            ...
            <div><div style={{fontWeight:700,fontSize:13}}>김철수 부장</div><div style={{fontSize:11,color:'var(--fg-muted)'}}>인사기획팀 · Lv.4</div></div>
          </div>
        </div>
        <div className="card">
...
const EvaluatorFeedbackPage = () => {
  const list = [
    {to:'박민수',title:'2026년 상반기 인사평가 체계 개선',score:4,date:'4/20',content:'체계 전환 과제를 주도적으로 이끌었고, 현업 인터뷰 20회 이상을 진행해 실행 가능한 정책으로 만들었습니다.'},
    {to:'정수진',title:'HR 데이터 대시보드 구축 지원',score:3,date:'4/18',content:'요구사항 정리가 깔끔했음. BI팀 리드의 피드백도 긍정적.'},
    {to:'강하늘',title:'기여도 평가 시스템 사용자 온보딩',score:3,date:'4/5',content:'교육 프로그램의 구조는 좋았으나 평가자 그룹 대상 심화 세션이 부족했던 점 아쉬움.'},
  ];
  return <div style={{padding:24,display:'flex',flexDirection:'column',gap:16}}>
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-end'}}>
      ...
      <div style={{display:'flex',gap:6}}>
        ...
        <button className="btn btn-ghost btn-sm">박민수</button>
        <button className="btn btn-ghost btn-sm">정수진</button>
        <button className="btn btn-ghost btn-sm">강하늘</button>
      </div>
    </div>
    <div style={{display:'flex',flexDirection:'column',gap:10}}>
      {list.map((f,i)=>(
