...
- [x] **S-3** 행 단위 접근제어: 평가/과업/피드백/알림 read 라우트에 "본인 / 담당 평가자 / HR" 검사(IDOR 차단 — `GET /api/evaluation/:id`:6000, `/api/evaluations/employee/:id`:5960 등). 백도어 사번 `H1000005`(`AuthContext.tsx:21-23`) 제거 — DB `available_roles`만 신뢰.
...
- 보안: 공용 비밀번호 '1234'(types/index.ts:310)·무인증 API 80개(server.js:2285)·IDOR(evaluation/:id)·reset 자기신고 가드(server.js:260,4902)·백도어 H1000005(AuthContext:21) — **Phase S 전이 배포 차단 사유**.
