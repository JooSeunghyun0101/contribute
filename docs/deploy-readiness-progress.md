...
- [x] **S-3** 행 단위 접근제어: 평가/과업/피드백/알림 read 라우트에 "본인 / 담당 평가자 / HR" 검사(IDOR 차단 — `GET /api/evaluation/:id`:6000, `/api/evaluations/employee/:id`:5960 등). 백도어 사번 `H9000026`(`AuthContext.tsx:21-23`) 제거 — DB `available_roles`만 신뢰.
...
- 보안: 공용 비밀번호 '1234'(types/index.ts:310)·무인증 API 80개(server.js:2285)·IDOR(evaluation/:id)·reset 자기신고 가드(server.js:260,4902)·백도어 H9000026(AuthContext:21) — **Phase S 전이 배포 차단 사유**.
...
- 2026-06-12 라이브 후속: 매트릭스 저장 500(사용자 보고) — DB 진단으로 원인 확정: `POST /api/setting`이 setting_data를 raw 바인딩하는데 node-pg는 **배열 파라미터를 JSON이 아닌 PG 배열 리터럴로 직렬화** → 루트가 배열인 매트릭스(number[][])만 `invalid input syntax for type json`(FAQ·기대수준은 객체라 통과, E-1a 스모크도 객체라 못 잡음). 수정: JSON.stringify+`$3::jsonb` 캐스트+필수필드 400. 라이브 DB에서 배열 raw=실패/stringify=성공 재현으로 검증, nodemon 자동 재시작 확인. 동반 확인: settings 유니크 제약(user_id,setting_type)은 라이브 DB에 존재, `/api/prompt/*` 404는 기본 프롬프트 폴백 정상동작(노이즈).
- 2026-06-12 라이브 후속: 평가자 점수표 '매트릭스 분포' 미연동(사용자 확인 중 발견) — 히트맵 셀 숫자가 **저장된 과업 점수 평균**이라 매트릭스 개정이 반영 안 됨(다른 화면은 라이브 매트릭스). ScoreTablePage 히트맵을 useEvaluationMatrix 연결로 전환: 셀 숫자=현재 매트릭스 기준 점수, 농도=평가된 과업 수(기존 유지), 툴팁 '평균'→'기준 점수'. typecheck·build EXIT 0. 매트릭스 검증 항목 화면 확인 마저 진행.
