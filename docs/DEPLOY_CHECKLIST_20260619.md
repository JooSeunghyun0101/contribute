...
- [x] **발령 변경→원복 재현 (점수 유지) — 실데이터 라이브 E2E 통과**(2026-06-22). 박종현(2402031, 2025완료 9건 sum20 + 2026 12건 sum17, 전 엔트리 ah_id NULL=옛 광역매칭 트리거조건)에 `PUT /api/employee/:id`(평가자 이상준1010099→유치우1412194) → `evaluator-assignment-history/:id/cancel`(원복) 라이브 실행. 결과: 원본 21엔트리 **바이트 동일 보존**(2025 sum20·2026 sum17 그대로)·새 draft평가만 cancelled·employee.evaluator_id 1010099 복귀·X→X 0. 백업 `hr-db-backup-20260622_e2e-pre.dump` + 삽입행 정밀삭제로 footprint 사전상태 복원(employees.updated_at 1건만 cosmetic 잔여). 상세는 아래 "정적+스캔 재검증" 말미.
...
**방법**: 현재 작업트리 코드(60afcb9)를 **별도 인스턴스(:5055)** 로 기동(원본 :5000 미신뢰 — 패치 이전 기동 가능성) → 실데이터 계정으로 API 직접 호출. 테스트 전 백업(`hr-db-backup-20260622_smoke-pre.dump`), 쓰기 시나리오는 **정밀 원복**(before-파일 기준 신규 알림 2건 삭제·평가상태·타임스탬프 복원) 후 사전 스냅샷과 **완전 일치 확인**(employees 773·notif 69·점수 무변조). 대상: 평가 `8993561c`, 피평가자 김은혜(0311008)/평가자 유치우(1412194), 비교군 김준호(2001041), HR=admin.

| # | 시나리오 | 기대 | 실제 |
|---|---|---|---|
| 0 | 미인증 `GET /api/evaluator-mappings` (쿠키 없음) | 401 | **401** ✅ 전역 게이트 |
| 1 | 피평가자가 평가자 명의로 점수 위조 `PUT /api/task-evaluation-entry` | 차단 | **423** *"another evaluator owns entries"* ✅ (evaluator_id 세션 강제 → 본인 소유 엔트리 없음). DB: 0311008 엔트리 미생성·유치우 점수 4 불변 |
| 1b | 쓰기에 `X-Requested-With` 누락 | 403 | **403** ✅ CSRF 게이트 |
| 2 | 배정 평가자 정상 채점 (status=submitted 상태) | 200 | **200** evaluator_id=1412194/score=4 ✅ |
| 3a | 피평가자가 `evaluation_status='completed'` | 403 | **403** ✅ P0-7 |
| 3b | 피평가자가 `'submitted'` | 200 | **200** ✅ |
| 4 | 평가자가 `?evaluatorId=<타인>`로 피드백 조회 | 본인 것만 | **세션 강제**(타인=본인=4건 동일, 김준호 39건 유출 없음) ✅ P0-6 |
...
| 6 | 알림 sender_id 사칭(`9999999`) | 세션 강제·직원 미생성 | 저장 sender_id=**1412194**, employees **773 불변**·가짜 사번 0 ✅ P0-5 |
...
정수진(1000005) 케이스: 김철수 완료 → 홍길동 변경 → 변경 취소(원복) 후 **완료 점수 소실 + 이력 "김철수→김철수"**. 실 DB 추적으로 3개 버그 확정·수정.
...
- **전수점검**: 피해 = **정수진 1명**(과업 9·이력 1행). 다른 직원 영향 0. 원본 점수는 취소 entry에 보존돼 있었음.
...
- ✅ **발령 생명주기 전수점검 완료(2026-06-19)**:
  ...
  - **DB 전수 스캔**: 점수 소실/타기간 취소 피해 = **정수진 1명뿐(복구 완료)**. completed-but-cancelled 2건 모두 정수진 2026 테스트 잔여(cancelled라 비표시).
...
| **AH2** 변경취소가 타 기간 점수까지 취소 | `transferEvaluatorEntriesForCorrectionScoped` server.js:1088 — 대상 엔트리를 `WHERE evaluation_id=$1` 로 **평가 단위 스코프**(전역 `assignment_history_id IS NULL` 광역매칭 제거) | cancelled 엔트리 전체 **2건뿐, 둘 다 정수진(1000005) 2026 알려진 잔여**(score 4·3). 타 기간 취소 피해 0 |
...
실데이터 대상 **박종현(2402031)** — 변경 전 상태: 평가자 이상준(1010099), 2025평가(completed, 9엔트리 합20)·2026평가(in-progress, 12엔트리 합17), **21엔트리 전부 `assignment_history_id` NULL**(옛 BUG-AH2 광역매칭이 잡던 cross-period 조건). 별도 :5055 인스턴스에 HR(admin)로 라이브 호출:

1. `PUT /api/employee/2402031` `{evaluator_id:'1412194', evaluation_period_id:'<2026>'}` → **200**, employee.evaluator_id=1412194, 새 'change' 이력 + **새 2026 draft 평가** 생성(기존 평가는 불변).
2. `POST /api/evaluator-assignment-history/<H>/cancel` → **200**, employee.evaluator_id **1010099 복귀**, 새 draft 평가 `record_status=cancelled`.
