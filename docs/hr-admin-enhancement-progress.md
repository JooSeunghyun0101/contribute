...
- [ ] **F-C2b-prep** 샘플 데이터 현실적 재구성. **사용자 결정(2026-06-09): 2025·2026 모두 샘플 → 최대한 실제처럼 재구성 후 F-C2b 옵션2 구현, 데모 caveat 불필요.** 평가자 잠재성향(관대도·변별력) 다양화·연도간 드리프트(기질 vs 급변)·전보 코호트·현실적 갭/점수 분포·의견 품질 다양화. ⚠ DB 쓰기(대량) — 백업 선행, 멱등 스크립트, 스키마/매트릭스 규칙 준수.
- [대기] **F-C2b** 평가 품질 점검 — **종단**(전보 코호트·전년 드리프트·코호트-잔차·기질vs급변). F-C2b-prep 후 구현(옵션2). **데모 caveat 없이** 실데이터처럼.
- [ ] **CLEANUP** F-C2a의 데모/합성 데이터 caveat 제거(사용자: 불필요). 완료율 등 기능적 표기는 유지.
