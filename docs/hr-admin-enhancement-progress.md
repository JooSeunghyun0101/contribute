...
- [ ] **F-C2a** 평가 품질 점검 — **횡단+무결성**(사용자 결정: 점진). 무결성 자동 플래그(빈 평가·가중치 합≠100·평가의견 미작성 등) + 변별력(갭 분산)×분포 건전성(횡단, 단일 기간 작동) + 드릴다운 + 재검토 요청(F-B2.2 `requestReturn` 재사용). 설계원칙 §2.1 엄수(판정 아님·HR 전용·인원 병기·소표본 회색·중립색). 갭=점수−성장레벨.
- [대기] **F-C2b** 평가 품질 점검 — **종단**(전보 코호트·전년 드리프트·코호트-잔차). ✅ **2기간 확인됨**: 2025-annual(closed, 695건 완결=안정 기준선) + 2026-annual(active, 754건/730완료=잠정). 종단 viable → **F-C2a 완료 후 진행**. caveat: 2026은 진행중(완료건만 대상), 2025 기준선.
