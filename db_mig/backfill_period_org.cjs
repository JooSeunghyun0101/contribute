...
const OVERRIDE_2025 = { '1504315': 'O042', '1601012': 'O045' }; // 안성환→소비자금융기획팀, 이성식→여신심사1팀
...
async function main() {
  ...
  try {
    ...
    // 최민선: 2025 없어야(제외), 2026 리스크관리팀/연결회계팀
    const choi = updates.filter((u) => evs.find((e) => e.id === u.id)?.evaluatee_id === '1409117').map((u) => { const e = evs.find((x) => x.id === u.id); return { y: e.evaluation_year, team: u.team, div: u.div, dept: u.dept, corp: u.corp }; });
    console.log('\n[최민선 1409117]'); choi.forEach((x) => console.log('  ' + JSON.stringify(x)));
    // 안성환/이성식
    for (const sid of ['1504315', '1601012']) { const us = updates.filter((u) => evs.find((e) => e.id === u.id)?.evaluatee_id === sid).map((u) => { const e = evs.find((x) => x.id === u.id); return { y: e.evaluation_year, team: u.team, dept: u.dept, div: u.div }; }); console.log(`[${sid}]`); us.forEach((x) => console.log('  ' + JSON.stringify(x))); }
