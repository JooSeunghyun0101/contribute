...
// 컬럼/테이블이 배포 DB 에 빠지면 해당 기능이 500 으로 깨진다(예: /reopen 의 returned_at,
// /api/org-kpis/tree 의 org_kpis, /api/drafts 의 ui_drafts). 부팅 시 멱등 IF NOT EXISTS 로 보강 —
// Render/내부망/로컬 어디서든 재시작만으로 스키마가 맞춰진다.
// (이미 있으면 no-op. 신규 마이그레이션 컬럼·테이블은 여기에 누적한다.)
async function ensureRuntimeSchema() {
  ...
  try {
    ...
    `);
    // 조직 KPI (db_mig/add_org_kpis*.sql) — 없으면 KPI 화면 활성화 시 /api/org-kpis/tree 500.
    await pool.query(`
    ...
    `);
    // 뒤 마이그레이션 컬럼 — 구 org_kpis 가 이미 있고 컬럼만 빠진 경우까지 보강.
    await pool.query(`
    ...
    `);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_org_kpis_period_org ON org_kpis(evaluation_period_id, org_level, org_key)`);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_org_kpis_parent ON org_kpis(parent_kpi_id)`);
    await pool.query(`
    ...
    `);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_task_kpi_alloc_kpi  ON task_kpi_allocations(kpi_id)`);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_task_kpi_alloc_task ON task_kpi_allocations(task_uuid)`);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_task_kpi_alloc_eval ON task_kpi_allocations(evaluation_id)`);
    // UI 임시저장(자동저장, db_mig/add_ui_drafts.sql) — 없으면 /api/drafts 500(클라 localStorage 폴백만 남음).
    await pool.query(`
    ...
    `);
    // ── 대상자/매칭 엑셀 업로드 경로 스키마 자가복구 (구 스냅샷 배포 DB) ──
    // employees 후행 컬럼(특히 department_id_source 는 render-schema.sql 에도 누락)·import batch 기간 귀속·
    // import row/employees 의 org 4단계·org_structure 스냅샷 테이블. 하나라도 없으면 대상자 업로드가
    // 500(column/relation does not exist) 로 깨진다. 각 문장을 개별 try 로 감싸 하나가 실패해도 나머지는 진행.
    const uploadHeals = [
      `ALTER TABLE employees
         ADD COLUMN IF NOT EXISTS growth_level           integer,
      ...
      `ALTER TABLE employee_profile_import_rows
         ADD COLUMN IF NOT EXISTS org_corporation text,
         ADD COLUMN IF NOT EXISTS org_division    text,
         ADD COLUMN IF NOT EXISTS org_department  text,
         ADD COLUMN IF NOT EXISTS org_team        text`,
      `ALTER TABLE matching_import_rows
         ADD COLUMN IF NOT EXISTS org_corporation text,
         ADD COLUMN IF NOT EXISTS org_division    text,
         ADD COLUMN IF NOT EXISTS org_department  text,
         ADD COLUMN IF NOT EXISTS org_team        text`,
      `ALTER TABLE evaluator_assignment_history ADD COLUMN IF NOT EXISTS evaluation_period_id uuid`,
    ...
    for (const ddl of uploadHeals) {
      try {
        ...
        console.error('[schema] 업로드 경로 보강 실패(건너뜀):', healErr.message);
      }
    }
    console.log('[schema] 런타임 스키마 보강 확인 완료');
  } catch (err) {
...
const createDraftEvaluationForEmployeeAssignment = async (
  ...
  // 조직(부서코드+4단계) 파생 — 없으면 매칭 재업로드로 생긴 새 단계 평가행이 조직 NULL 로
  // 남아 KPI 조직장 산정(전원 수렴)에서 구성원이 이탈하고 그 조직이 화면에서 사라진다.
  // 우선순위: ① 단계의 부서ID(deptCodeHint)를 그 기간 org_structure 스냅샷으로 번역(발령의
  // 새 소속 반영) → ② 같은 기간 기존 활성 평가행에서 승계 → ③ employees.department_id 를
  // 스냅샷으로 번역(대상자 업로드의 org 파생과 동일 규칙). 셋 다 없으면 NULL(기여도 업로드가 채움).
  let org = null;
  if (activePeriod?.id) {
    const snapOf = async (deptCode) => {
      if (!deptCode) return null;
      const { rows: snap } = await client.query(
        `SELECT dept_code, org_corporation, org_division, org_department, org_team
           FROM org_structure
          WHERE dept_code = $1 AND evaluation_period_id = $2
          LIMIT 1`,
        [deptCode, activePeriod.id]
      );
      return snap[0] ?? null;
    };
    org = await snapOf(deptCodeHint);
    if (!org) {
      const { rows: prior } = await client.query(
        `SELECT evaluatee_dept_code AS dept_code,
                evaluatee_org_corporation AS org_corporation, evaluatee_org_division AS org_division,
                evaluatee_org_department AS org_department, evaluatee_org_team AS org_team
           FROM evaluations
          WHERE evaluatee_id = $1 AND evaluation_period_id = $2
            AND COALESCE(record_status, 'active') = 'active'
            AND evaluatee_org_corporation IS NOT NULL
          ORDER BY updated_at DESC
          LIMIT 1`,
        [employee.employee_id, activePeriod.id]
      );
      org = prior[0] ?? null;
    }
    if (!org) org = await snapOf(employee.department_id ?? null);
  }
  ...
  const { rows } = await client.query(
    `
      INSERT INTO evaluations (
        ...
        evaluation_period_id,
        evaluatee_dept_code,
        evaluatee_org_corporation,
        evaluatee_org_division,
        evaluatee_org_department,
        evaluatee_org_team,
        last_modified,
        created_at,
        updated_at
      )
      VALUES ($1,$2,$3,$4,$5,'draft','active',$6,$7,$8,$9,$10,$11,$12,NOW(),NOW(),NOW())
      RETURNING *
    `,
    [
      ...
      deptCodeHint ?? org?.dept_code ?? null,
      org?.org_corporation ?? null,
      org?.org_division ?? null,
      org?.org_department ?? null,
      org?.org_team ?? null,
    ]
  );
...
const buildMatchingStagesForEmployee = (rows) => {
  ...
  for (const r of sorted) {
    ...
    if (last && last.evaluatorId === r.evaluator_id) {
      ...
      if (r.department_id) last.deptId = r.department_id; // 단계 최종 소속(부서ID)
      continue;
    }
    stages.push({
      ...
      deptId: r.department_id ?? null, // 신규 평가행의 조직(4단계) 파생용
    });
  }
  return stages;
};
...
const reconcileEmployeeMatchingStages = async (
  ...
  if (Number.isFinite(periodYear) && stages.length === 0) {
    ...
    if (carry?.evaluatorId) {
      stages = [{ startDate: `${periodYear}-01-01`, evaluatorId: carry.evaluatorId, deptId: carry.deptId ?? null }];
    }
  }
  ...
  for (let si = 0; si < stages.length; si += 1) {
    ...
    const evaluation = await createDraftEvaluationForEmployeeAssignment(
      client, employee, periodId, fs.deptId ?? null
    );
...
app.post('/api/matching-imports', requireHr, async (req, res) => {
  ...
  try {
    ...
    // ── 조직 백필: 이 기간에 조직(evaluatee_org_*) 없이 남은 활성 평가행 치유 ──────
    // 과거 버전 reconcile 이 조직 없이 평가행을 만들어 KPI 조직장 산정(전원 수렴)에서
    // 구성원이 이탈하던 문제의 데이터 측 복구. 같은 파일을 재업로드만 해도 복구된다.
    // 행→단계 매칭: 행의 이력 발령일(changed_at) == 파일 단계 startDate → 그 단계의
    // 부서ID를 그 기간 org_structure 스냅샷으로 번역. 발령자의 행 여러 개도 단계별로
    // 정확히 귀속된다(형제 행 승계는 발령자에서 엉뚱한 조직을 복사할 수 있어 쓰지 않음).
    // 폴백: employees.department_id → 스냅샷. 스냅샷이 없으면 건너뜀(기여도 업로드가 채움).
    if (periodForReconcile?.id && stagesBuiltByEmployee.size > 0) {
      const { rows: orglessRows } = await client.query(
        `SELECT e.id, e.evaluatee_id, to_char(h.changed_at, 'YYYY-MM-DD') AS stage_date
           FROM evaluations e
          ...
          WHERE e.evaluation_period_id = $1
            ...
            AND e.evaluatee_org_corporation IS NULL
            AND e.evaluatee_id = ANY($2::text[])`,
        [periodForReconcile.id, [...stagesBuiltByEmployee.keys()]]
      );
      const snapCache = new Map();
      const snapOf = async (deptCode) => {
        if (!deptCode) return null;
        if (snapCache.has(deptCode)) return snapCache.get(deptCode);
        const { rows: snap } = await client.query(
          `SELECT dept_code, org_corporation, org_division, org_department, org_team
             FROM org_structure
            WHERE dept_code = $1 AND evaluation_period_id = $2
            LIMIT 1`,
          [deptCode, periodForReconcile.id]
        );
        snapCache.set(deptCode, snap[0] ?? null);
        return snap[0] ?? null;
      };
      for (const row of orglessRows) {
        const empStages = stagesBuiltByEmployee.get(row.evaluatee_id) ?? [];
        const stage =
          empStages.find((s) => s.startDate === row.stage_date) ??
          [...empStages].reverse().find((s) => s.startDate && row.stage_date && s.startDate <= row.stage_date) ??
          empStages[empStages.length - 1] ??
          null;
        let deptCode = stage?.deptId ?? null;
        if (!deptCode) {
          const { rows: er } = await client.query(
            'SELECT department_id FROM employees WHERE employee_id = $1 LIMIT 1',
            [row.evaluatee_id]
          );
          deptCode = er[0]?.department_id ?? null;
        }
        const snap = await snapOf(deptCode);
        if (!snap) continue;
        await client.query(
          `UPDATE evaluations
              SET evaluatee_dept_code       = COALESCE(evaluatee_dept_code, $2),
                  evaluatee_org_corporation = $3,
                  evaluatee_org_division    = $4,
                  evaluatee_org_department  = $5,
                  evaluatee_org_team        = $6,
                  updated_at                = NOW()
            WHERE id = $1`,
          [row.id, deptCode, snap.org_corporation, snap.org_division, snap.org_department, snap.org_team]
        );
      }
    }
...
app.put('/api/employee/:id', requireHr, async (req, res) => {
  ...
  try {
    ...
    // 동기화한다(2026-07-07 사용자 보고: 특정 직원 레벨을 3으로 저장해도 화면이 계속 4 —
...
// 조직 경로(법인·본부)가 달라도 부모의 하위 조직으로 본다(예: A법인 부서장이 직접 평가하는
// B법인 팀). 어느 한쪽이라도 조직장 미산정이면 false(경로 규칙만 적용).
...
//   예) 팀T1·팀T2=팀장L(직접 평가), 부서D=부서장M(소속 등록 시 — 팀장L은 그의 평가자
//       부서장M이 부 구성원이 되므로 자동 탈락), 본부B=없음(라인 미수렴 → HR 탭에서만 등록).
...
const computeKpiLeadership = async (periodId) => {
  ...
  // 이전 조직의 전원 수렴이 깨진다(예: 팀T1→타사 팀T2 발령자 때문에 팀T1 조직장 소실).
...
//             경계를 넘으므로(예: A법인 부서장이 B법인 팀장을 직접 평가) 조직 경로 접두만으로는
...
const getKpiScope = async (req, periodId) => {
  ...
  // 예) 팀T1 팀원의 체인 최상단이 부서장이면 옆 팀(팀T2) KPI 도 보인다.
...
app.get('/api/org-kpis/parent-candidates', requireHrOrEvaluator, async (req, res) => {
  ...
  try {
    ...
    // 조직의 KPI 도 후보에 포함(예: B법인 팀 → A법인 부서, 법인이 달라도 연결 가능).
