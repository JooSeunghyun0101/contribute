...
export const useEvaluationData = (employeeId: string) => {
  ...
  // Fetch real employee data from PostgreSQL and update state
  useEffect(() => {
      ...
      try {
        ...
        if (emp) {
          setEmployeeInfo({
            ...
            growthLevel: emp.growth_level || 1,
          });
          // Update evaluation data with fetched employee info
          setEvaluationData(prev => ({
            ...prev,
            evaluateeName: emp.name,
            evaluateePosition: emp.position,
            evaluateeDepartment: emp.department,
            growthLevel: emp.growth_level || 1,
          }));
        }
      } catch (e) {
  ...
  const handleMethodClick = (taskId: string, method: string) => {
    ...
    if (method === '기여없음') {
      ...
      updatedTask.score = getMatrixScore(method, updatedTask.contributionScope, matrix) ?? updatedTask.score;
    }
  ...
  const handleScopeClick = (taskId: string, scope: string) => {
    ...
    if (scope === '기여없음') {
      ...
      updatedTask.score = getMatrixScore(updatedTask.contributionMethod, scope, matrix) ?? updatedTask.score;
    }
      ...
      for (const task of evaluationData.tasks) {
        ...
        if (currentFeedback.trim()) {
          ...
          if (otherFeedbacks.length > 0) {
            try {
              const duplicateCheck = await checkSimilarFeedback(currentFeedback, otherFeedbacks, user?.name ?? '');
              if (duplicateCheck.isDuplicate) {
                duplicateWarnings.push(`"${task.title}": ${duplicateCheck.summary}`);
              }
            } catch (error) {
