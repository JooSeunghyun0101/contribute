...
// 다운로드 파일명 통일: 제목_평가기간_다운로드일자(.xlsx). 평가기간 없으면 생략.
export const buildExportFileName = (title: string, periodLabel?: string | null, suffix = ''): string => {
  const safe = (s: string) => s.replace(/[\\/:*?"<>|]/g, '_').replace(/\s+/g, ' ').trim();
  const parts = [safe(title)];
  if (periodLabel && periodLabel.trim()) parts.push(safe(periodLabel));
...
const withPeriodSnapshot = (employee: Employee, ev: Evaluation): Employee => ({
  ...employee,
  department: ev.evaluatee_department || employee.department,
  position: ev.evaluatee_position || employee.position,
  growth_level: ev.growth_level ?? employee.growth_level,
  org_corporation: ev.evaluatee_org_corporation ?? employee.org_corporation,
  org_division: ev.evaluatee_org_division ?? employee.org_division,
  org_department: ev.evaluatee_org_department ?? employee.org_department,
  org_team: ev.evaluatee_org_team ?? employee.org_team,
});

const sortByDeptNameId = (list: Employee[]) =>
  [...list].sort(
    (a, b) =>
      (a.department ?? '').localeCompare(b.department ?? '') ||
      (a.name ?? '').localeCompare(b.name ?? '') ||
...
export const downloadEmployeeProfileUploadWorkbook = async (
  ...
  const fileName = writeWorkbook(wb, buildExportFileName('평가대상자_업로드양식', options.periodLabel));
...
export const downloadFullEvaluationDataWorkbook = async (
  options: { includePastEvaluations?: boolean; periodLabel?: string | null } = {},
): Promise<ExportResult> => {
  const result = await createFullEvaluationDataWorkbook(options);
  const suffix = options.includePastEvaluations === false ? '' : '_이전평가포함';
  const fileName = writeWorkbook(result.wb, buildExportFileName('평가데이터_전체', options.periodLabel, suffix));
...
export const downloadDepartmentMembersWorkbook = (
  ...
  const fileName = writeWorkbook(wb, buildExportFileName(`부서명단_${departmentName}`, periodLabel));
  return { fileName, memberCount: members.length };
};

// ── 조직정보 스냅샷 다운로드 ──────────────────────────────────────────────────
// 특정 평가기간의 org_structure 행(부서코드→4단계 조직)을 엑셀로 내려받는다.
...
const ORG_STRUCTURE_HEADERS = ['부서코드', '부서명', '법인', '본부', '부', '팀', 'T-Level', '조직종류'];

export const downloadOrgStructureWorkbook = (periodLabel: string, rows: OrgStructureRow[]): string => {
  const wb = XLSX.utils.book_new();
  const objRows = rows.map((r) => ({
    부서코드: r.dept_code,
    부서명: r.dept_name ?? '',
    법인: r.org_corporation ?? '',
    본부: r.org_division ?? '',
    부: r.org_department ?? '',
    팀: r.org_team ?? '',
    'T-Level': r.t_level ?? '',
  ...
  appendObjectSheet(wb, '조직정보', ORG_STRUCTURE_HEADERS, objRows);
  return writeWorkbook(wb, buildExportFileName('조직정보', periodLabel));
};
...
const INDIVIDUAL_GUIDE_ROWS: unknown[][] = [
  ...
  ['발령 다중평가자', '발령(이동)으로 평가자가 여럿인 경우 현재 평가자 기준 1건을 표시합니다. 정상 케이스입니다.'],
];
