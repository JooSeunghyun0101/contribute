...
const EvaluationAccordionCard = ({
  ...
  // 평가가 '확정'된 상태인지(완료/잠금). 되돌려서 '평가중'이 되면 점수가 남아 있어도 확정 아님.
  const isFinalized = evaluationStatus === 'completed' || evaluationStatus === 'locked';
                  ...
                  <div className="flex items-center gap-2" style={{ marginBottom: 10 }}>
                    {mode === 'create' ? (
                      ...
                      <>
                        ...
                        {/* 라벨은 '점수 유무'가 아니라 '평가 확정 여부'로 판정한다.
                            되돌려서 '평가중'이면 점수가 남아 있어도 '평가 중'으로 표시(완료 오인 방지). */}
                        <Pill
                          tone={
                            isFinalized
