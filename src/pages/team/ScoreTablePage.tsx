...
// 팀원 org 필터는 '그 평가 기간'의 조직 기준(evaluatee_org_*), 없으면 현재 employee.org_* 폴백.
...
const ScoreTablePage = () => {
  ...
  // 메인 점수표/카드/도넛/히트맵 = "선택 기간" 기준 담당 팀원.
  // 그 기간 평가의 배정 평가자(evaluation.evaluator_id)가 본인인 레코드만 — 기간을 바꾸면
  // 그 기간의 실제 담당 팀원으로 전환된다. (이전엔 현재 담당만 보여, 25년 조회 시에도
  // 26년 담당 팀원이 나오던 문제 해소. 현재 기간에서는 결과 동일.)
  const records = useMemo(
    () =>
      rosterRecords.filter(
        (r) =>
          matchesOrgNodes(recordOrg(r), orgFilter) &&
          String(r.evaluation?.evaluator_id ?? '') === evaluatorId,
      ),
    [rosterRecords, orgFilter, evaluatorId],
  );
      ...
      <PageHeader
        ...
        filters={
          allRecords.length > 0 ? (
            <>
              <OrgChecklist
                items={allRecords.map((r) => recordOrg(r))}
                value={orgFilter}
