...
const EvaluatorSchedulePage = () => {
  const { user } = useAuth();
  // 선택 기간에 이 평가자가 담당한 팀원(현재 + 기간 중 이관)을 일정에 표시.
  // (이전엔 현재 담당만 보여, 25년 조회 시에도 26년 담당 팀원이 나오던 문제 해소.)
  const { selectedPeriod } = useEvaluationPeriod();
  const { current, former, isLoading, error } = useEvaluatorPeriodRoster(
    user?.employeeId || '',
    selectedPeriod?.id ?? null,
    true,
  );
  const records = useMemo(() => [...current, ...former], [current, former]);
