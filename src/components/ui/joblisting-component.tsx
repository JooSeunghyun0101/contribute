        ...
        {activeItem ? (
          <>
            <div className="group absolute inset-0 z-10 grid place-items-center">
              <motion.div
                className="bg-background flex h-fit w-[90%] cursor-pointer flex-col items-start gap-4 overflow-hidden border p-4 shadow-sm"
                ref={ref}
        ...
        <div className="relative flex w-full flex-col items-center gap-4 px-2">
          {jobs.map((role) => (
            <motion.div
              ...
              key={role.company}
              className="group bg-background flex w-full cursor-pointer flex-row items-center gap-4 border p-2 shadow-sm md:p-4"
              onClick={() => {
                setActiveItem(role)
