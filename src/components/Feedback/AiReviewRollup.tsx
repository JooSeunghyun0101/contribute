...
import { aiReviewService, type AiReviewRollup as Rollup } from '@/lib/services';
          ...
          <div>
            ...
            {data.items.length === 0 ? (
              ...
              (() => {
                ...
                const detail =
                  selected && map.get(selected.ev)
                    ? map.get(selected.ev)!.items.filter((it) => selected.type === '합계' || norm(it.ai_type) === selected.type)
                    : [];
                ...
                return (
                  <>
                    ...
                    {selected && (
                      <div className="sd-card" style={{ marginTop: 10, padding: 0, overflow: 'hidden' }}>
                        <div style={{ padding: '8px 12px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
                          <span style={{ fontWeight: 800 }}>
                            {selected.ev} · {selected.type} <span style={{ color: 'var(--fg-muted)', fontWeight: 600 }}>{detail.length}건</span>
                          </span>
